...
async def discover_agents():
    ...
    return GatewayAgentsResponse(
        agents=agents, gateway_url=discovery_service.gateway_url, count=len(agents)
    )
...
async def get_local_discovery_info():
    return local_discovery.get_discovery_info()


@router.post("/local/scan")
async def scan_local_config():
    agents = local_discovery.read_agents_from_config()
    for agent in agents:
        existing = await discovery_service.register_agent(agent)
    return {
        "status": "ok",
        "agents_found": len(agents),
        "agents": agents,
        "config_path": str(local_discovery.find_config_file())
        if local_discovery.find_config_file()
        else None,
    }
...
async def get_local_config():
    config_path = local_discovery.find_config_file()
    if not config_path:
...
async def configure_discovery(config: DiscoveryConfig):
    discovery_service.update_config(
        gateway_url=config.gateway_url, api_key=config.api_key, enabled=config.enabled
    )
...
async def trigger_discovery():
    agents = await discovery_service.discover_agents_from_gateway()
    return {"status": "ok", "agents_found": len(agents), "agents": agents}
...
async def sessions_spawn(request: SessionSpawnRequest):
    result = await discovery_service.sessions_spawn(
        target_agent_id=request.target_agent_id, task=request.task
    )

    if not result:
        raise HTTPException(status_code=500, detail="Failed to spawn session")

    return {"status": "ok", "session_id": result.get("session_id")}
...
async def sessions_history(session_id: str):
    ...
    return {"status": "ok", "session_id": session_id, "history": history}
