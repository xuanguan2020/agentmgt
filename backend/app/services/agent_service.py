...
class AgentService:
    async def create_agent(self, agent_data: AgentCreate) -> Agent:
        ...
        await memory_cache.add_activity(
            "agent_registered", {"agent": agent.model_dump()}
        )
    ...
    async def update_agent(
        self, agent_id: str, updates: AgentUpdate
    ) -> Optional[Agent]:
    ...
    async def heartbeat(
        self, agent_id: str, heartbeat: HeartbeatRequest
    ) -> Optional[Agent]:
        updates = {
            ...
            if agent.status != AgentStatus.OFFLINE:
                ...
                if time_diff > 120:
                    await self.update_agent(
                        agent.id, AgentUpdate(status=AgentStatus.OFFLINE)
                    )
